  ...
  return (
    <div className={`relative inline-block ${className}`} ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#111522] border border-white/10 hover:border-purple-500/40 text-xs font-mono font-bold text-gray-200 transition-all active:scale-95 shadow-sm"
      >
      ...
      {open && (
        <div className="absolute left-0 mt-2 w-72 p-2.5 rounded-2xl dropdown-menu z-50 space-y-1.5 shadow-2xl font-mono text-xs border border-white/10 bg-[#0F1420]/95 backdrop-blur-xl">
          {/* Header */}
          <div className="flex items-center justify-between px-2.5 py-1.5 border-b border-white/10">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
              <Filter className="w-3 h-3 text-purple-400" /> Account Portfolio Filter
            </span>
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-300 font-bold">
          ...
          >
            <div className="flex items-center gap-2.5">
              <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${
                isAllSelected ? "bg-purple-600 border-purple-500 text-white" : "border-gray-500"
              }`}>
                {isAllSelected && <Check className="w-3 h-3" />}
              </div>
              <span className="font-bold">Select All Accounts</span>
            </div>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-xl border flex items-center justify-between transition-all ${
                    isChecked
                      ? "bg-white/10 border-purple-500/30 text-white font-medium"
