...
export function formatHoldDuration(totalMs: number): string {
...
export function calculateTradeStats(trades: Trade[], initialBalance = 10000): TradeStats {
  ...
  sortedTrades.forEach((t) => {
    ...
    if (t.holdDurationMs && t.holdDurationMs > 0) {
      totalHoldTimeMs += t.holdDurationMs;
      validHoldCount++;
    }
  ...
  const avgHoldMs = validHoldCount > 0 ? totalHoldTimeMs / validHoldCount : 0;
  ...
  return {
    ...
    balance: parseFloat((initialBalance + totalNetProfit).toFixed(2)),
...
export function getEquityCurve(trades: Trade[], initialBalance = 10000): EquityPoint[] {
  ...
  const points: EquityPoint[] = [
    {
      date: sorted.length > 0 ? format(parseISO(sorted[0].openTime), "MMM dd") : "Start",
      pnl: 0,
...
export function getDailyPnLSeries(trades: Trade[]): DailyPnLPoint[] {
  if (!trades || trades.length === 0) return [];
...
export function getMonthlyPnLSeries(trades: Trade[]): MonthlyPnLPoint[] {
  if (!trades || trades.length === 0) return [];
...
export function getDrawdownSeries(trades: Trade[], initialBalance = 10000): DrawdownPoint[] {
  if (!trades || trades.length === 0) return [];
  const sorted = [...trades].sort((a, b) =>
    compareAsc(parseISO(a.closeTime), parseISO(b.closeTime))
  );
...
export function getProfitDistributionSeries(trades: Trade[]): ProfitDistributionPoint[] {
  if (!trades || trades.length === 0) return [];
  const ranges = [
  ...
  trades.forEach((t) => {
    const r = ranges.find((rg) => t.pnl >= rg.min && t.pnl < rg.max);
    if (r) r.count++;
  });
...
export function getSymbolPerformance(trades: Trade[]) {
  if (!trades || trades.length === 0) return [];
  const map = new Map<
    string,
